...
def _handle_keycloak_error(response: requests.Response, context: str):
    ...
    description = error_map.get(
        response.status_code, f"Erro inesperado ao {context}: {error_details}"
    )
...
def _handle_request_exception(
    exception: requests.exceptions.RequestException, context: str
):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if response_get.status_code == 200:
                ...
                role_create_dict["id"] = role_data.get(
                    "id"
                )  # Adiciona o ID da role na resposta
...
def update_role(access_token: str, role_id: str, role_update: RoleUpdate):
    ...
    try:
        ...
        response = requests.put(
            url, json=role_update.dict(), headers=get_headers(access_token)
        )
...
def patch_role(access_token: str, role_id: str, role_update: RoleUpdate):
    ...
    try:
        ...
        url = f"{KEYCLOAK_URL}/admin/realms/{REALM_NAME}/roles-by-id/{role_id}"
        response = requests.put(
            url, json=existing_role_data, headers=get_headers(access_token)
        )
