...
def test_create_role_failure(mock_create_role):
    ...
    mock_create_role.side_effect = APIException(
        status_code=409,
...
def test_patch_role_success(mock_patch_role):
    ...
    response = client.patch(
        "/roles/123",
        json={"name": "admin", "description": "Updated role"},
        headers=MOCK_AUTH_HEADER,
    )
...
def test_patch_role_failure(mock_patch_role):
    ...
    mock_patch_role.side_effect = APIException(
        status_code=400,
...
def test_put_role_success(mock_update_role):
    ...
    response = client.put(
        "/roles/123",
        json={"name": "admin", "description": "Replaced role"},
        headers=MOCK_AUTH_HEADER,
    )
...
def test_put_role_failure(mock_update_role):
    ...
    mock_update_role.side_effect = APIException(
        status_code=400,
        description="Erro ao substituir role",
        error_code="RS-400-02",
        source="RoleService",
    )
