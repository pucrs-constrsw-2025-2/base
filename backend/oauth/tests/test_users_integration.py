...
def test_create_user_success(auth_token):
    ...
    response = client.post("/users", json=payload, headers=auth_token)
...
def test_create_user_failure_existing_user(auth_token):
    ...
    assert "Conflito: Usuário ou email já existente. (Detalhe: User exists with same email)" in response.json().get(
        "detail"
    )
