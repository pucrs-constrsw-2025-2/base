...
use oauth::get_all_roles;

#[actix_rt::test]
async fn test_get_all_roles_endpoint_with_auth() {
    let app = test::init_service(
        App::new().service(
            web::resource("/roles").route(web::get().to(get_all_roles))
        )
    ).await;
...
#[actix_rt::test]
async fn test_get_all_roles_endpoint_missing_auth() {
    let app = test::init_service(
        App::new().service(
            web::resource("/roles").route(web::get().to(get_all_roles))
        )
