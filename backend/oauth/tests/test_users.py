...
def test_create_user_success(mock_create_user):
    ...
    response = client.post("/users", json=payload, headers=MOCK_AUTH_HEADER)
...
def test_create_user_failure(mock_create_user):
    ...
    mock_create_user.side_effect = APIException(
        status_code=409,
        description="Usuário já existe",
        error_code="RS-409-01",
        source="UserService",
    )
...
def test_patch_update_password_failure(mock_update_password_service):
    ...
    mock_update_password_service.side_effect = APIException(
        status_code=400,
        description="Erro ao atualizar a senha.",
        error_code="RS-400-01",
        source="UserService",
    )
...
def test_put_user_success(mock_update_user_data):
    ...
    response = client.put("/users/123", json=payload, headers=MOCK_AUTH_HEADER)
...
def test_put_user_failure(mock_update_user_data):
    ...
    mock_update_user_data.side_effect = APIException(
        status_code=400,
