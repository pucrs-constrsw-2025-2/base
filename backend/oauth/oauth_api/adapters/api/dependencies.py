...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]) -> dict[str, Any]:
    try:
        ...
        if not rsa_key:
            raise InvalidTokenError(description="Chave pública (kid) não encontrada no JWKS.")

        payload = jwt.decode(
            token, rsa_key, algorithms=[settings.KEYCLOAK_TOKEN_ALGORITHM],
            audience=settings.KEYCLOAK_CLIENT_ID, issuer=f"{settings.KEYCLOAK_SERVER_URL}/realms/{settings.KEYCLOAK_REALM}"
        )
        return payload
